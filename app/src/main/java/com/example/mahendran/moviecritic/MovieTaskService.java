...
public interface MovieTaskService {
    ...
    @GET("3/movie/{sort_by}")
    Observable<movieResponse> discoverMovies(@Path("sort_by") String sortBy, @Query("api_key") String apiKey);

    @GET("3/movie/{id}/videos")
    Observable<Trailers> findTrailers(@Path("id") long movieId, @Query("api_key") String apiKey);

    @GET("3/movie/{id}/reviews")
    Observable<Reviews> findReviews(@Path("id") long movieId, @Query("api_key") String apiKey);
}
